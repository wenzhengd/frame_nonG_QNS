...
read_theta=np.load('result_D3_qns_theta.npy')
read_n = np.load('result_D3_qns_n.npy')
read_rho = np.load('result_D3_qns_rho.npy')
read_matrix = np.load('result_D3_qns_Matrix.npy')


TIME_NUMBER = 100 # partition of [0,T] 
GAMMA_FLIP = 0.02 # flipping rate gamma
G_COUPLING =1 # coupling
...
def f_ctrl_x(i, t):
	#i is C_ctrl index, t is time
	window = 2 if t==T else int(t/(T/2))+1
	return nx(i,window)*theta(i,window)

def f_ctrl_y(i, t):
	#i is C_ctrl index, t is time
	window = 2 if t==T else int(t/(T/2))+1
	return ny(i,window)*theta(i,window)	

def f_ctrl_z(i, t):
	#i is C_ctrl index, t is time
	window = 2 if t==T else int(t/(T/2))+1
	return nz(i,window)*theta(i,window)
...
def H_sb(i, j, t):
	...
	t is time
	"""
	tmp = U_ctrl(i,t).dag() * tensor(sigmaz(), RTN(j,t)*identity(2)) * U_ctrl(i,t)
	f_sb_x = lambda t,args: np.trace(tmp*tensor(sigmax(), identity(2)))/4
	f_sb_y = lambda t,args: np.trace(tmp*tensor(sigmay(), identity(2)))/4
	f_sb_z = lambda t,args: np.trace(tmp*tensor(sigmaz(), identity(2)))/4
	return [[tensor(sigmax(),identity(2)), f_sb_x],\
...
def obs_x_avg(i):
	...
	exp_f =lambda j: 1/2*np.trace(U_sb(i,j,T) * tensor(f_map(init_list[i]),identity(2))\
								* U_sb_dag(i,j,T)*tensor(sigmax(), identity(2)) )
		obs_x_each = Parallel(n_jobs=-1, verbose=1)(delayed(exp_f)(j) for j in range (ENSEMBLE_SIZE) )
...
np.save('experiment_D3_qns_observables', obs_avg_simu)
